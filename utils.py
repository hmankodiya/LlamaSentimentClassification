...
def get_dataset_config(dataset_config: dict):
    ...
    try:
        ...
        dataset_language = dataset_config.pop("dataset_language")
        # Log the configuration
        logger.info(f"Loaded dataset configuration: {dataset_config}")

        return dataset_path, dataset_language, dataset_config
...
def get_model_config(config):
    ...
    try:
        ...
        if "model_name" not in model_config:
            raise KeyError(
                "Missing required keys in 'model_config': 'model_name'"
            )
